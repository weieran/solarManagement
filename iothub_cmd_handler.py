...
def do_start():
    print("Start Solar Management!")
    # run the application with "app start solar.sh"
    os.system("app start solar.sh")
    return True


def do_stop():
    print("Stop Solar Management!")
    os.system("app stop solar.sh")
    return True
    ...
    async with session.direct_method_requests() as method_requests:
        async for method_request in method_requests:
            if method_request.name == "start":
                ...
                status = 200
                print("'start' was completed with result: {}".format(result))
            elif method_request.name == "stop":
                print("Direct Method request received for 'bar'. Invoking.")
                result = do_stop()
                payload = {"result": result}
                status = 204
                print("'stop' was completed with result: {}".format(result))
            else:
                payload = {}
                status = 400
